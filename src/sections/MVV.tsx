...
const cards = [
  ...
  },
  {
    ...
    title: 'Valores',
    bg: 'bg-[#c9956b]',
    titleColor: 'text-[#0d0f14]',
    textColor: 'text-[#3d2e20]',
    description: null,
    list: [
...
export default function MVV() {
  return (
    <section id="mvv" className="w-full bg-[#0d0f14] py-24 lg:py-32 relative overflow-hidden">
      {/* Background seal */}
      ...
      <div className="w-full max-w-[1440px] mx-auto px-10 lg:px-20 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-0">
          {cards.map((card, i) => (
            <div
              key={card.title}
              className={`${card.bg} p-10 lg:p-12 border border-[#2a2520]/40 ${i === 2 ? '' : 'border-r-0'} relative group`}
            >
              {/* Top accent */}
              <div className="flex items-center justify-between mb-8">
                <h3 className={`font-serif text-[28px] font-semibold ${card.titleColor || 'text-[#f5f0e8]'}`}>
                  {card.title}
                </h3>
              ...
              {card.description && (
                <p className={`${card.textColor} font-sans text-[14px] leading-[1.7]`}>
                  {card.description}
                </p>
              )}
