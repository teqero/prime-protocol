        ...
        <div className="max-w-[900px] mx-auto space-y-3">
          {faqs.map((faq, i) => (
            <div
              ...
              style={{ borderColor: 'var(--pp-border)', backgroundColor: 'var(--pp-bg-2)' }}
            >
              <button
                onClick={() => setOpenIndex(openIndex === i ? null : i)}
                className="w-full flex items-center justify-between p-6 lg:p-8 text-left group"
              >
                <span className="font-serif text-[18px] lg:text-[20px] font-normal pr-4 group-hover:text-[#c9956b] transition-colors"
                      style={{ color: 'var(--pp-text)' }}>
                  {faq.question}
                </span>
                <ChevronDown
